...
storageWarning.hidden = appStorage.persistent;
appStorage.onFailure = () => { storageWarning.hidden = false; };
...
const supabaseClient = supabase.createClient(SUPABASE_URL, SUPABASE_PUBLISHABLE_KEY, {
  auth: { persistSession:true, autoRefreshToken:true, detectSessionInUrl:true, storage:appStorage }
});
...
let useCases;
let records = [];
let settings = { target:528, break:FIXED_BREAK_MINUTES, theme:appStorage.getItem(THEME_PREFERENCE_KEY) === "dark" ? "dark" : "light", manualBalances:{} };
...
const rememberedEmail = appStorage.getItem(REMEMBERED_EMAIL_KEY) || "";
...
function updateForecast() {
  ...
  const date=$("#work-date").value, cycle=HoursCycle.rangeForMonth(HoursCycle.monthForDate(date)), editingId=$("#editing-id").value;
  const basisRecords=(historyRange ? filteredRecords() : recordsInRange(cycle)).filter((record)=>record.id!==editingId);
...
function recordsInRange(range) { return range ? records.filter((record)=>record.date>=range.from && record.date<=range.to).sort((a,b)=>b.date.localeCompare(a.date)) : []; }
function selectedRange() { return historyRange || HoursCycle.rangeForMonth($("#month-filter").value); }
function filteredRecords() {
  return recordsInRange(selectedRange());
}
function historyPeriodLabel() {
  const range=selectedRange();
  return range ? range.from.split("-").reverse().join("/")+" a "+range.to.split("-").reverse().join("/") : "Período inválido";
}
function historyFilenameSuffix() {
  const range=selectedRange();
  return range ? range.from+"-a-"+range.to : "periodo-invalido";
}
...
function renderBackupReminder() {
  const lastBackup=Number(appStorage.getItem(backupReminderKey("last"))) || 0;
  const snoozedUntil=Number(appStorage.getItem(backupReminderKey("snooze"))) || 0;
...
function applyImportedPeriod(dates) {
  ...
  $("#month-filter").value=HoursCycle.monthForDate(historyRange.to);
...
function render() {
  ...
  $("#empty-state").textContent=historyRange ? "Nenhum registro neste período." : "Nenhum registro neste ciclo.";
  ...
  $("#photo-history-title").textContent=historyRange ? "Fotos do período" : "Fotos do ciclo";
  ...
  $("#balance-period-label").textContent=historyRange ? "Saldo do período" : "Saldo do ciclo";
  $("#balance-period-dates").textContent=historyPeriodLabel();
  $("#history-title").textContent=historyRange ? "Registros do período" : "Registros do ciclo";
  $("#simulator-description").textContent=historyRange
    ...
    : "Informe saldos anteriores ou ajustes que ainda não aparecem nos registros deste ciclo.";
...
form.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    const savedCycle=HoursCycle.monthForDate(record.date);
    if (!historyRange && savedCycle && $("#month-filter").value!==savedCycle) {
      $("#month-filter").value=savedCycle;
      loadManualBalance();
    }
...
function applyTheme() { document.documentElement.dataset.theme=settings.theme; appStorage.setItem(THEME_PREFERENCE_KEY,settings.theme); $("#theme-toggle").textContent=settings.theme==="dark" ? "☀️" : "🌙"; document.querySelector('meta[name="theme-color"]').content=settings.theme==="dark" ? "#0d1321" : "#3157d5"; }
...
function buildPdf() {
  ...
  pages.forEach((page,index)=>{
    ...
    if (index===0) {
      const cards=[
        {label:historyRange ? "SALDO DO PERIODO" : "SALDO DO CICLO",value:signed(totals.balance),bg:"0.91 0.93 1",fg:"0.12 0.25 0.68"},
    ...
    if (!page.length) commands.push(text(historyRange ? "Nenhum registro no periodo pesquisado." : "Nenhum registro no ciclo selecionado.",42,rowY-12,10,"F1","0.38 0.43 0.52"));
...
$("#backup-reminder-later").addEventListener("click",()=>{
  appStorage.setItem(backupReminderKey("snooze"),String(Date.now()+BACKUP_SNOOZE_DAYS*DAY_MS));
  renderBackupReminder();
});
...
$("#json-file").addEventListener("change",async(event)=>{
  ...
  try {
    ...
    for (const [month,balance] of Object.entries(manualBalances)) appStorage.setItem(`${MANUAL_BALANCE_KEY}:${loadedUserId}:${month}`,JSON.stringify({ positive:manualDurationInput(balance.positive), negative:manualDurationInput(balance.negative) }));
